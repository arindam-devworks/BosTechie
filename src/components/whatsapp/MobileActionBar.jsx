...
export default function MobileActionBar({ onSave, onPreview, onSend, isSending }) {
    return (
        <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 p-3 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] z-[100] flex items-center gap-3">
            <button
                onClick={onSave}
                className="flex flex-col items-center justify-center gap-1.5 p-2 px-4 text-slate-500 hover:text-slate-900 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors"
            >
                <Save size={18} />
